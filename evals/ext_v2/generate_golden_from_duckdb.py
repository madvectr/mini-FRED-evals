#!/usr/bin/env python3
"""Deterministic truth-spec sampler for MVES ext_v2."""
...
DEFAULT_OUT = ROOT / "evals" / "ext_v2" / "golden.jsonl"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=36,
        help="Approximate number of truth specs to sample per series (biased toward non-point transforms).",
    )
    parser.add_argument(
        "--series",
        nargs="+",
...
def allocate_counts(per_series: int) -> Dict[str, int]:
    ...
    if per_series <= 0:
        return counts
    total_weight = sum(TRANSFORM_WEIGHTS.values())
    remainder = per_series
    for transform in TRANSFORMS:
        weight = TRANSFORM_WEIGHTS[transform]
        share = int(per_series * weight / total_weight)
        if share == 0 and weight > 0:
            share = 1
        counts[transform] = share
        remainder -= share
    bias_order = sorted(
        TRANSFORMS,
        key=lambda name: TRANSFORM_WEIGHTS[name],
        reverse=True,
    )
    idx = 0
    while remainder > 0:
        counts[bias_order[idx % len(bias_order)]] += 1
        remainder -= 1
        idx += 1
    return counts
            ...
            for series_id in series_ids:
                ...
                for transform, requested in counts.items():
                    ...
                    if len(produced) < requested:
                        logging.warning(
                            "[ext_v2] %s/%s %s cases for %s (requested=%s)",
                            len(produced),
                            requested,
                            transform,
                            series_id,
...
def _print_summary(summary: Dict[str, Dict[str, Dict[str, int]]]) -> None:
    print("[ext_v2] Generation summary:")
    total = 0
    ...
    print(f"[ext_v2] Total truth specs: {total}")
