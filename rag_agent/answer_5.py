"""Fifth iteration of the Mini-FRED agent with Phi-4 Mini assisted parsing."""
...
from src.parse import DATE_REQUIRED_ERROR, MISSING_SERIES_ERROR, WINDOW_REQUIRED_ERROR, parse_question  # noqa: E402
...
from . import llm_parser  # noqa: E402
...
VALID_TRANSFORMS = {"point", "yoy", "mom", "ma", "max", "min"}
LLM_TRANSFORM_THRESHOLD = 0.45
LLM_REFUSAL_THRESHOLD = 0.55
LLM_REFUSAL_ERROR = (
    "Question appears underspecified—please provide the series name, date, or window."
)
...
def run(
    ...
    *,
    ...
    parsed = _apply_llm_hints(parsed, normalized_question)
    ...
    if getattr(parsed, "_llm_refusal", False):
...
def _apply_llm_hints(parsed, question: str):
    hints = llm_parser.infer_slots(question) if hasattr(llm_parser, "infer_slots") else None
    overrides: Dict[str, str] = {}
    if not hints:
        parsed._llm_overrides = overrides
        return parsed

    def _maybe_override(attr: str, value: Optional[str]):
        if not value:
            return
        normalized = _normalize_llm_date(value)
        if attr in {"window_start", "window_end"}:
            normalized = normalized or _parse_date_token(value)
        elif attr == "date":
            normalized = normalized or _parse_date_token(value)
        if normalized:
            setattr(parsed, attr, normalized)
            overrides[attr] = "llm"
            if attr == "date":
                _clear_error(parsed, DATE_REQUIRED_ERROR)
            if attr in {"window_start", "window_end"} and parsed.window_start and parsed.window_end:
                _clear_error(parsed, WINDOW_REQUIRED_ERROR)

    if hints.series_guess and not parsed.series_id:
        parsed.series_id = hints.series_guess.strip().upper()
        overrides["series_id"] = "llm"
        _clear_error(parsed, MISSING_SERIES_ERROR)

    if hints.transform and hints.transform in VALID_TRANSFORMS:
        if hints.confidence >= LLM_TRANSFORM_THRESHOLD or parsed.transform == "point":
            parsed.transform = hints.transform
            overrides["transform"] = "llm"

    if hints.periods and hints.periods > 0:
        parsed.periods = hints.periods
        overrides["periods"] = "llm"

    _maybe_override("date", hints.date)
    _maybe_override("window_start", hints.window_start)
    _maybe_override("window_end", hints.window_end)

    if hints.should_refuse and hints.confidence >= LLM_REFUSAL_THRESHOLD:
        parsed.errors = list(dict.fromkeys(parsed.errors + [LLM_REFUSAL_ERROR]))
        parsed._llm_refusal = True

    parsed._llm_overrides = overrides
    return parsed


def _normalize_llm_date(token: Optional[str]) -> Optional[str]:
    if not token:
        return None
    token = token.strip()
    iso_full = re.fullmatch(r"\d{4}-\d{2}-\d{2}", token)
    if iso_full:
        return iso_full.group(0)
    iso_month = re.fullmatch(r"\d{4}-\d{2}", token)
    if iso_month:
        return f"{iso_month.group(0)}-01"
    month_year = _parse_date_token(token)
    if month_year:
        return month_year
    return None


def _clear_error(parsed, target: str) -> None:
    parsed.errors = [err for err in parsed.errors if err != target]
...
def _build_parse_trace(parsed) -> Dict[str, Optional[str]]:
    return {
        ...
        "errors": list(parsed.errors),
        "llm_overrides": getattr(parsed, "_llm_overrides", {}),
        "llm_refusal": getattr(parsed, "_llm_refusal", False),
    }
